...
$ arp -a
```
Lists [[ARP & ND#ARP (Address Resolution Protocol)]] table of current system.
...
$ arp HOST_IP
```
See if the host you're looking for is on your [[ARP & ND#ARP (Address Resolution Protocol)]] entries. So sometimes, when you can't `ping` to a host, it doesn't necessary mean the [network layer](obsidian://open?vault=Computers%20-%20Infinity%20Stones&file=Networking%2FConceptual%2FThe%20Four%20Layers) is broken, maybe you don't have a [[ARP & ND#ARP (Address Resolution Protocol)]] entry for that machine. You can know if nothing comes out of the `arp HOST_IP`, the address is listed as `incomplete`, or `missing`. If such situation arise, then it's your datalink layer that's broken.
