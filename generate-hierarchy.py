...
def generate_hierarchy(vault_dir: str):
    ...
    for md_file in vault_path.rglob('*.md'):
        ...
        keys_to_purge = ['parent', 'nav_order']
        if is_readme:
        ...
        insertions = []
        insert_parent = grand_parent if is_readme else parent

        if is_readme:
            insertions.append(f"title: {parent}\n" if parent else "title: Home\n")

        if insert_parent:
            insertions.append(f"parent: {insert_parent}\n")
